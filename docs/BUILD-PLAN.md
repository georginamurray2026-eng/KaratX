...
**Acceptance criteria:**
- **AMENDED by ADR-014 and OQ-20 — the signature cannot be met as originally written.** It said `(candles, timeframe, boundaryConfig) => candles`. The function is pure, and that half is unchanged and non-negotiable. What changed is the RETURN: it yields an `AggregateResult` carrying the derived bars **plus** `periodsExpected`, `periodsRejectedForMissingConstituents`, `unknownInstants`, `unexpectedBarsExcluded` and `expectsBarAtCalls`. **The reason is OQ-20.** The job must assert `periodsExpected == rows_written + periods_rejected`, and that assertion is only meaningful if `periodsExpected` is computed **independently, from the calendar**. The pure function is the only place that HOLDS the calendar — returning a bare array would force the worker to recompute the expectation from the outputs it just received, at which point the assertion becomes an identity that cannot fail for any input, passes on every run for ever, and reads to a future session as evidence that the decomposition balanced. A decomposition that cannot fail is a confirmation wearing a decomposition's clothes. Amended here rather than reinterpreted quietly, following the precedent ADR-013 consequence 3 set for T1.3
