...
- Backfill is cheap: 6.6 years in **47 requests, ~6 minutes**, within a free tier of 800 credits/day and 8/minute. **CORRECTED 2026-09-04 — THAT FIGURE IS DERIVED, NOT MEASURED, AND NO 6.6-YEAR BACKFILL HAS BEEN RUN.** It is 6.6 × 35,071 ÷ 5,000 rounded up, and 47 ÷ 8 for the minutes; only the 5,000 bars/request and 35,071 bars/year underneath it were measured, and the 8/min and 800/day limits are vendor documentation that has never been tested. A second derivation using the **measured** weekday density for 2020–2024, where weekend bars were counted at zero, gives **35–36 requests and ~4.5 minutes**; 47 applies the 24/7 density throughout and is the conservative one. **The conclusion is unaffected on either derivation** — 5.9% and 4.5% of a single day's credits — which is why this is a provenance correction and not a reversal.
...
**2. An unscoped `WHERE timeframe = '1D'` will mix provenances once step 8 writes aggregates.** `candles` already holds 1,449 vendor `1D` bars on a UTC-day boundary (obligation 49), and will hold derived `1D` bars on an **18:00-New-York** session boundary. **These are different objects that will sit in the same table with the same `timeframe` value**, separated only by `provider_id`. Any query that reads daily bars without filtering on provider is wrong, and will look right. Obligation 49's parity work is the first consumer that must respect this.

**CORRECTED 2026-09-09 — THIS SENTENCE SAID 17:00, AND THE SHIPPED CODE USES 18:00.** `packages/core/src/timeframes/aggregate.ts` requires `dailyBoundaryLocal` to equal the `weekly_open` rule's `localStart` and throws when they disagree; migration `0004_calendar_measured_against_twelve_data.sql` moved `weekly_open` from 17:00 to 18:00 New York on measured Twelve Data evidence, so the derived session day rolls at 18:00. Measured: derived `1D` sits at **22:00Z and 23:00Z** — 18:00 New York under EDT and EST — **302 bars over the golden-fixture window, 212 and 90.** The 17:00 figure named the CHART's daily boundary, which is what the golden fixture uses (**211 bars at 21:00Z and 88 at 22:00Z**, 299 in total), and not the calendar this system aggregates on. **Two different boundaries, an hour apart, and this sentence conflated them.** They still correspond by date: the fixture-to-derived date-key join matches **299 of 299, with zero fixture-only dates.**
